# -*- coding: utf-8 -*-
"""
Created on Wed Feb  7 10:21:24 2018
...
x_list = []
with open('table4') as fin:     #  таблица №4 из книги МАХ расходы по годам
    for line in fin:
        year, x = line.split(' ')
...
with open('table5', 'w') as fout:      #  таблица №5 из книги
    for k in range(1, 73):
        x = x_list[k]            # 4-столбец   x расходы воды в 1000м3
        kn = round(k/73, 3)      # 2-столбец   к/(N+1)
        y = phi(kn)
...
x1 = sum(plt_x)
x3 = (x1**2)/72
sx = ((x2-x3)/72)**(1/2)    #  средний квадратический наблюдений
